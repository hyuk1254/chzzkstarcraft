...
<style>
    ...
    .stApp p, .stApp span, .stApp label, .stApp h1, .stApp h2, .stApp h3, .stApp h4, .stApp h5, .stApp h6, .stApp div { color: #e2e8f0 !important; }
    ...
    .main-title-container { padding-top: 35px !important; margin-bottom: 5px !important; clear: both !important; }
    .main-title {
    ...
    /* 1층 박스: 팀 멤버 배정 스타일 */
    .team-box-row { display: flex !important; flex-direction: row !important; gap: 20px !important; width: 100% !important; margin-bottom: 20px !important; }
    .broadcast-team-card { background-color: #14161d; border: 2px solid #2d3139; border-radius: 16px; padding: 20px; flex: 1 !important; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.4); }
    .broadcast-player-container { display: flex !important; flex-direction: row !important; justify-content: center !important; gap: 15px !important; flex-wrap: wrap !important; margin-top: 15px; }
    .broadcast-player-card { background-color: #1c1e26; border: 1px solid #2d3139; border-radius: 12px; padding: 12px; text-align: center; width: 130px; }
    .broadcast-avatar-img img { width: 90px !important; height: 90px !important; object-fit: cover !important; border-radius: 12px !important; margin: 0 auto 8px auto; }
    
    /* 2층 박스: 사용맵 나열 와이드 스타일 */
    .use-map-container-box { background-color: #14161d; border: 2px solid #2d3139; border-radius: 16px; padding: 15px 20px; margin-bottom: 35px; width: 100% !important; }
    .map-timeline-row { display: flex !important; flex-direction: row !important; justify-content: center !important; gap: 15px !important; flex-wrap: nowrap !important; overflow-x: auto !important; }
    .map-timeline-box { background-color: #1c1e26; border: 1px solid #2d3139; border-radius: 12px; padding: 10px; text-align: center; width: 150px !important; flex-shrink: 0 !important; }
    .map-timeline-img img { width: 100% !important; height: 95px !important; object-fit: cover !important; border-radius: 6px !important; }
    
    /* 3층 박스: 와이어프레임 실현 수평 1:1 대진표 공식 */
    .match-board-card { background-color: #181a20; border: 1px solid #2d3139; border-radius: 16px; padding: 25px; margin: 0 auto 30px auto !important; max-width: 650px !important; box-shadow: 0 6px 24px rgba(0, 0, 0, 0.6); }
    .match-board-flex-row { display: flex !important; flex-direction: row !important; justify-content: space-between !important; align-items: center !important; width: 100% !important; }
    .match-side-player-block { text-align: center !important; width: 160px !important; flex-shrink: 0 !important; }
    .match-player-img img { width: 150px !important; height: 150px !important; object-fit: cover !important; border-radius: 12px !important; border: 2px solid #2d3139; margin: 0 auto; }
    .match-mid-text-container { text-align: center !important; flex-grow: 1 !important; display: block !important; }

    .draft-panel { background-color: #181a20; border: 1px solid #2d3139; border-radius: 12px; padding: 25px; margin-top: 35px !important; }
    .admin-match-text-row { background-color: #181a20; border: 1px solid #2d3139; border-radius: 8px; padding: 12px 20px; margin-bottom: 10px; }
    .race-icon-img { display: inline-block; width: 18px; height: 16px; object-fit: contain; vertical-align: middle; margin-right: 4px; }
    
    .stButton > button { padding: 3px 10px !important; font-size: 0.85rem !important; background-color: #1c1e26 !important; color: #e2e8f0 !important; border: 1px solid #2d3139 !important; transition: all 0.2s ease; }
    .stButton > button:hover { border-color: #00ffa3 !important; color: #00ffa3 !important; box-shadow: 0 0 8px rgba(0, 255, 163, 0.3); }
    .del-btn > div > button { padding: 2px 8px !important; font-size: 0.8rem !important; background-color: rgba(255, 85, 85, 0.15) !important; color: #ff5555 !important; border: 1px solid rgba(255, 85, 85, 0.3) !important; }
...
with tab_main:
    ...
    # 1. 1층 박스: [1팀 멤버] 와 [2팀 멤버] 묶음 박스 가로 정렬
    left_team_html = ""
    players_left = st.session_state.teams['Left_Team']['players']
    if players_left:
        for p_name in players_left:
            ...
            """
    else: left_team_html = "<p style='text-align:center; color:#64748b; margin-top:15px; width:100%;'>배정된 선수가 없습니다.</p>"

    right_team_html = ""
    players_right = st.session_state.teams['Right_Team']['players']
    if players_right:
        for p_name in players_right:
            p_row = df_players[df_players['name'] == p_name] if not df_players.empty else pd.DataFrame()
            race = p_row.iloc[0]['race'] if not p_row.empty else "미정"
            img = p_row.iloc[0]['img_url'] if not p_row.empty else ""
            right_team_html += f"""
            ...
            """
    else: right_team_html = "<p style='text-align:center; color:#64748b; margin-top:15px; width:100%;'>배정된 선수가 없습니다.</p>"

    st.markdown(f"""
    <div class='team-box-row'>
        <div class='broadcast-team-card'>
            <h3 style='color:#00ffa3; font-weight:bold; text-align:center; margin:0;'>🟢 {st.session_state.teams['Left_Team']['name']}</h3>
            <div class='broadcast-player-container'>{left_team_html}</div>
        </div>
        <div class='broadcast-team-card'>
            <h3 style='color:#00ffa3; font-weight:bold; text-align:center; margin:0;'>🔴 {st.session_state.teams['Right_Team']['name']}</h3>
            <div class='broadcast-player-container'>{right_team_html}</div>
        </div>
    </div>
    """, unsafe_allow_html=True)

    # 2. 2층 박스: [사용맵 나열] 가로형 단독 배너 (🚨 따옴표 이중 렌더링 충돌 결함 영구 수정 파트)
    if st.session_state.matches:
        ...
        for idx, match in enumerate(st.session_state.matches):
            ...
            # 파이썬 문자열 연산 내부에 쌍따옴표 정밀 격리
            timeline_html += "<div class='map-timeline-box'>"
            timeline_html += f"<p style='margin:0 0 5px 0; font-size:0.85rem; font-weight:bold; color:#00ffa3;'>SET {idx+1}</p>"
            timeline_html += f"<div class='map-timeline-img'><img src='{map_img}'></div>"
            timeline_html += f"<p style='margin:5px 0 0 0; font-size:0.85rem; font-weight:bold; color:#fff; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;'>{match['map']}</p>"
            timeline_html += "</div>"
            
        timeline_html += "</div>"
        
        # ⭕ unsafe_allow_html=True 엔진이 정상적으로 변수를 번역하도록 주입 완료
    ...
    # 3. 3층 박스: 와이어프레임 구조의 1:1 대진표 카드 출력 (수평 고정 렌더러)
...
# =========================================================
# 📝 [2번 탭] 실시간 대진 지명 및 대진 현황 (텍스트 제어 보드)
# =========================================================
if not broadcast_mode:
    with tab_draft:
